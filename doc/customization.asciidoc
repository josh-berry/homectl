...
example.hcpkg/
  ...
  *.trigger      <--- triggers will be discussed later
--------------------------------------------------------------------------------
================================================================================
...
currently not system-specific.

+refresh.trigger+::
    When a package is enabled, or when an already-enabled package is updated via
    +hc refresh+, homectl will look in the top level of each package directory
    for a file called +refresh.trigger+.  If this file exists and is executable,
    homectl will run it.

+disable.trigger+::
    Similarly, when a package is disabled, homectl will search for and run a
    +disable.trigger+ executable file.

At present, these triggers are run in a non-interactive context, so they should
not expect to have access to user input.  They are called with no parameters and
